...
##########################
# DAr de altaA A USUARIOS#
##########################

# Altas Masivas

# 1. Creamos el fichero .csv
# 2. importamos el fichero
# 3. creamos una estructura foreach
# 4. ejecutar
#################################
Get-Content .\altas-usuarios.csv

$usuarios = Import-Csv .\altas-usuarios.csv

Foreach ($i in $usuarios)
{
$clave = ConvertTo-SecureString $i.contra -AsPlainText -Force
New-LocalUser $i.cuenta -Password $clave -Description $i.descripcion -FullName $i.NCompleto -AccountNeverExpires 
# todo lo que agreguemos a mayores que no sale en el fichero csv, como el -AccountNeverExpires que hemos puesto, tiene que ser para todos los usuarios, si no no lo puedo poner. Por ejemplo el grupo vamos a tener que ponerlo aparte.
Add-LocalGroupMember -Group Usuarios -Member $i.cuenta


}

# y ahora ejecutaríamos todo desde la creación de la variable $usuarios 


# y ahora los eliminamos a todos (ya tendríamos la variable usuarios con el archivo csv importado, si no pues habría que volverla a importar):
...
# y ejecutaríamos, y comprobaríamos con Get-LocalUser qué usuarios tendríamos.