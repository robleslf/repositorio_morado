...
-- Índice en especie de la tabla ANIMAL para agilizar la búsqueda de animales según su especie.
CREATE INDEX indice_especie_ANIMAL ON ANIMAL (especie);
...
CREATE TABLE ZONA (
    ...
    superficie DECIMAL(8,2) UNSIGNED NOT NULL COMMENT "Superficie de la zona en metros cuadrados, siendo la superficie mínima 10 metros cuadrados y la máxima 999999,99 metros cuadrados.",
...
-- Índice en especie de la tabla ZONA para agilizar la búsqueda de zonas según su tipo.
CREATE INDEX indice_tipo_ZONA ON ZONA (tipo);
...
-- Índice en clave foránea codigo_animal en la tabla VIVE para mejorar el rendimiento de consultas que involucren búsquedas por código de animal.
CREATE INDEX indice_codigo_animal_vive ON VIVE (codigo_animal);
...
CREATE TABLE FORMA_VETERINARIO (
    ...
    fecha_final_formacion DATE NULL COMMENT "Fecha en la que finalizó o finalizará la formación del veterinario.",
...
CREATE TABLE MANTIENE (
    ...
    fecha_fin DATETIME NULL COMMENT "Fecha y hora en la que el empleado de mantenimiento dejó de ocuparse de esta zona.",
...
-- Índice en especie de la tabla ANIMAL para agilizar la búsqueda de animales según su especie.
CREATE INDEX indice_gravedad_AFECCION ON AFECCION (gravedad);
